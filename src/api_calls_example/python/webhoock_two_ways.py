...
def get_token():
    ...
    try:
        auth_att = requests.post(
            url=f"{auth_url}", auth=(user, password),
            headers=auth_header, verify=False).json()['Token']
...
event_resp = requests.get(
    url=f"{event_url}", headers=headers, verify=False).json()
...
event_sub_resp = requests.post(
    url=f"{ev_sub_url}", headers=headers,
